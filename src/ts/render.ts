...
export function showReleaseNotes(): void {
  createModalContainer().innerHTML = components.modal(
    components.releaseNotes(getReleases())
  );
}
...
const components = {
  ...
  id: (id: ResourceId): string => `
        <span class="id">
            ${id.prefixes
              .map(
                (prefix) => `<span class="id-segment prefix">${prefix}</span>`
              )
              .join("")}
  ...
  action: (action: Action): string => `
        <li class="${action.type}">
            <div class="summary" onclick="accordion(this)">
                ...
                ${
                  action.changes
                    ? components.changeCount(action.changes.length)
                    : ""
                }
            </div>
